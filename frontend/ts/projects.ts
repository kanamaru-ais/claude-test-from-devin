...
export async function updateProject(id: string, name: string): Promise<Response> {
  return apiFetch(`/${id}`, {
    method: 'PUT',
    body: JSON.stringify({ name }),
  });
...
function init(): void {
  ...
  const projectNameError = document.getElementById('projectNameError') as HTMLSpanElement;
  ...
  projectTbody.addEventListener('click', (e) => {
    const editBtn  = (e.target as Element).closest<HTMLButtonElement>('.btn-edit');
    const deleteBtn = (e.target as Element).closest<HTMLButtonElement>('.btn-delete');
...
// ブラウザ環境（必要なDOM要素が存在する場合）のみ初期化を実行
if (typeof document !== 'undefined' && document.getElementById('projectTbody')) {
